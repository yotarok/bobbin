...
$ curl -O https://raw.githubusercontent.com/tensorflow/lingvo/master/lingvo/tasks/asr/wpm_16k_librispeech.vocab  # noqa: E501
"""
...
from bobbin.contrib import asrnn
...
class CtcAsrModel(nn.Module):
    ...
    def setup(self):
        self._encoder = asrnn.CnnEncoder()
    ...
    def __call__(
        ...
        encodes, encode_paddings = self._encoder(features, feature_paddings)
...
class EvalResults(bobbin.EvalResults):
    ...
    def preduce(self, axis_name: str) -> EvalResults:
        ...
        return EvalResults(start_time=self.start_time, end_time=self.end_time, **kwargs)
    ...
    def reduce(self, other: EvalResults) -> EvalResults:
        ...
        return EvalResults(start_time=start_time, end_time=end_time, **kwargs)
...
def _recover_padded_tokens(
    token_ids: np.ndarray, paddings: np.ndarray, blank_id: int = 0
):
    token_ids = np.asarray(token_ids).tolist()
    return [
        [lab for pad, lab in zip(pads, padded_ids) if pad < 0.5 and lab != blank_id]
...
def main(args: argparse.Namespace):
    ...
    if args.feature_normalizer is not None:
        normalizer = asrio.MeanVarNormalizer.load(args.feature_normalizer)

...
# This configuration is forked from that of chex (https://github.com/deepmind/chex)
# project. Thanks, chex team!
...
def _recursive_add_annotations_import():
    for path, _, files in os.walk("../bobbin/"):
        for file in files:
            if file.endswith(".py"):
...
def linkcode_resolve(domain, info):
    ...
    return "https://github.com/yotarok/bobbin/tree/master/bobbin/%s#L%d#L%d" % (
        os.path.relpath(filename, start=os.path.dirname(bobbin.__file__)),
        lineno,
        lineno + len(source) - 1,
    )
